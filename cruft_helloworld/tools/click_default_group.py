...
class DefaultGroup(click.Group):
    ...
    def parse_args(self, ctx, args):
        ...
        if (
            help_options
            and self.add_help_option
            and rest
            and any(s in help_options for s in rest)
        ):
            return super(DefaultGroup, self).parse_args(ctx, args)
        ...
        if (
            not rest
            and (ctx.protected_args or ['a'])[0][:1].isalnum()
            and not self.default_if_no_args
        ):
